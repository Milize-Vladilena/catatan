...
untuk id_rsa yang gw buat sendiri ada [disini](#drive).
...
- `~/.ssh/[id_rsa](#drive)_main` dan `[id_rsa](#drive)_main.pub`

- `~/.ssh/[id_rsa](#drive)_alt` dan `[id_rsa](#drive)_alt.pub`
...
```{.mardown filename="bash"}
cat ~/.ssh/id_rsa_main.pub
```

Salin hasilnya (mulai dari `ssh-rsa` ...) dan:

Masuk ke akun GitHub pertama (sesuai [id_rsa](#drive)_main)
